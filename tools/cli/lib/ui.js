...
class UI {
  ...
  async promptInstall(options = {}) {
    ...
    // Preflight: Check for legacy MDAN v4 footprints immediately after getting directory
    ...
    if (hasLegacyFolder || hasLegacyCfg) {
      ...
      await prompts.note(
        'Found a ".mdan"/"mdan" folder, or a legacy "_cfg" folder under the mdan folder -\n' +
          'this is from an old MDAN version that is out of date for automatic upgrade,\n' +
          ...
          '  1. Delete your current MDAN installation folder (.mdan or mdan)\n' +
          '  2. Run a fresh installation\n\n' +
          'If you do not want to start fresh, you can attempt to proceed beyond this\n' +
          'point IF you have ensured the mdan folder is named _mdan, and under it there\n' +
          'is a _config folder. If you have a folder under your mdan folder named _cfg,\n' +
      ...
      if (proceed === 'cancel') {
        await prompts.note('1. Delete the existing mdan folder in your project\n' + "2. Run 'mdan install' again", 'To do a fresh install');
        process.exit(0);
        return;
      }
      ...
      try {
        ...
        const cfgPath = path.join(mdanDir, '_cfg');
        if (await fs.pathExists(cfgPath)) {
          s.start('Renaming configuration folder...');
          const newCfgPath = path.join(mdanDir, '_config');
    ...
    // Check if there's an existing MDAN installation (after any folder renames)
    const hasExistingInstall = await fs.pathExists(mdanDir);
    ...
    if (hasExistingInstall) {
      ...
      const { existingInstall, mdanDir } = await this.getExistingInstallation(confirmedDirectory);
      const packageJsonPath = path.join(__dirname, '../../../package.json');
      const currentVersion = require(packageJsonPath).version;
      const installedVersion = existingInstall.version || 'unknown';

      // Check if version is pre beta
      const shouldProceed = await this.showLegacyVersionWarning(installedVersion, currentVersion, path.basename(mdanDir), options);
      ...
      // Common actions
      choices.push({ name: 'Modify MDAN Installation', value: 'update' });
      ...
      if (actionType === 'update') {
        ...
        if (options.customContent) {
          ...
          if (changeCustomModules) {
            ...
            const { Installer } = require('../installers/lib/core/installer');
            const installer = new Installer();
            const { mdanDir } = await installer.findMdanDir(confirmedDirectory);
  ...
  async promptToolSelection(projectDir, options = {}) {
    // Check for existing configured IDEs - use findMdanDir to detect custom folder names
    const { Detector } = require('../installers/lib/core/detector');
    const { Installer } = require('../installers/lib/core/installer');
    const detector = new Detector();
    const installer = new Installer();
    const mdanResult = await installer.findMdanDir(projectDir || process.cwd());
    const mdanDir = mdanResult.mdanDir;
   ...
   * @returns {Object} Object with existingInstall, installedModuleIds, and mdanDir
   */
  async getExistingInstallation(directory) {
    ...
    const detector = new Detector();
    const installer = new Installer();
    const mdanDirResult = await installer.findMdanDir(directory);
    const mdanDir = mdanDirResult.mdanDir;
    const existingInstall = await detector.detect(mdanDir);
    const installedModuleIds = new Set(existingInstall.modules.map((mod) => mod.id));

    return { existingInstall, installedModuleIds, mdanDir };
  }
  ...
  async selectAllModules(installedModuleIds = new Set()) {
    ...
    allOptions.push({ label: 'MDAN Core Module', value: 'core', hint: 'Core configuration and shared resources' });
    ...
    // Group 2: MDAN Official Modules (type: mdan-org)
    const officialModules = [];
    for (const mod of externalModules) {
      if (mod.type === 'mdan-org') {
  ...
  async getDefaultModules(installedModuleIds = new Set()) {
    ...
    // Add default-selected local modules (typically MDAN)
    for (const mod of localModules) {
    ...
    // If no defaults found, use 'mdan' as the fallback default
    if (defaultModules.length === 0) {
      defaultModules.push('mdan');
    }
  ...
  async displayDirectoryInfo(directory) {
    ...
    if (dirExists) {
      ...
      if (stats.isDirectory()) {
        ...
        if (files.length > 0) {
          // Check for any mdan installation (any folder with _config/manifest.yaml)
          const { Installer } = require('../installers/lib/core/installer');
          const installer = new Installer();
          const mdanResult = await installer.findMdanDir(directory);
          const hasMdanInstall =
            (await fs.pathExists(mdanResult.mdanDir)) && (await fs.pathExists(path.join(mdanResult.mdanDir, '_config', 'manifest.yaml')));

          const mdanNote = hasMdanInstall ? ` including existing MDAN installation (${path.basename(mdanResult.mdanDir)})` : '';
          await prompts.log.message(`Directory exists and contains ${files.length} item(s)${mdanNote}`);
        } else {
  ...
  async getConfiguredIdes(directory) {
    ...
    const installer = new Installer();
    const mdanResult = await installer.findMdanDir(directory);
    const existingInstall = await detector.detect(mdanResult.mdanDir);
  ...
  async handleCustomModulesInModifyFlow(directory, selectedModules) {
    ...
    const { Installer } = require('../installers/lib/core/installer');
    const installer = new Installer();
    const { mdanDir } = await installer.findMdanDir(directory);
   ...
   * @param {string} mdanFolderName - Name of the MDAN folder
   * @returns {Promise<boolean>} True if user wants to proceed, false if they cancel
   */
  async showLegacyVersionWarning(installedVersion, currentVersion, mdanFolderName, options = {}) {
    ...
    let warningContent;
    if (installedVersion === 'unknown') {
      warningContent = 'Unable to detect your installed MDAN version.\n' + 'This appears to be a legacy or unsupported installation.';
    } else {
      warningContent =
    ...
    warningContent +=
      ...
      '  1. Delete your current MDAN installation folder\n' +
      `     (the "${mdanFolderName}/" folder in your project)\n` +
      '  2. Run a fresh installation\n\n' +
      'Benefits of a fresh install:\n' +
      '  \u2022 Cleaner configuration without legacy artifacts\n' +
      '  \u2022 All new features properly configured\n' +
    ...
    if (proceed === 'cancel') {
      await prompts.note(
        `1. Delete the "${mdanFolderName}/" folder in your project\n` + "2. Run 'mdan install' again",
  ...
  async displayStatus(statusData) {
    const { installation, modules, availableUpdates, mdanDir } = statusData;
    ...
    if (availableUpdates.length > 0) {
      ...
      await prompts.log.message('Run \'mdan install\' and select "Quick Update" to update');
    } else {
