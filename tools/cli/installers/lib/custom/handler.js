...
class CustomHandler {
   ...
   */
  async install(customPath, mdanDir, config, fileTrackingCallback = null) {
    const results = {
      agentsInstalled: 0,
    ...
    try {
      // Create custom directories in mdan
      const mdanCustomDir = path.join(mdanDir, 'custom');
      const mdanAgentsDir = path.join(mdanCustomDir, 'agents');
      const mdanWorkflowsDir = path.join(mdanCustomDir, 'workflows');

      await fs.ensureDir(mdanCustomDir);
      await fs.ensureDir(mdanAgentsDir);
      await fs.ensureDir(mdanWorkflowsDir);

      // Process agents - compile and copy agents
      const agentsDir = path.join(customPath, 'agents');
      if (await fs.pathExists(agentsDir)) {
        await this.compileAndCopyAgents(agentsDir, mdanAgentsDir, mdanDir, config, fileTrackingCallback, results);
      ...
      if (await fs.pathExists(workflowsDir)) {
        await this.copyDirectory(workflowsDir, mdanWorkflowsDir, results, fileTrackingCallback, config);
      ...
      for (const entry of entries) {
        if (entry.isFile() && entry.name !== 'custom.yaml' && !entry.name.startsWith('.') && !entry.name.endsWith('.md')) {
          ...
          const targetPath = path.join(mdanCustomDir, entry.name);
   ...
   * @param {Object} results - Results object to update
   */
  async compileAndCopyAgents(sourceAgentsPath, targetAgentsPath, mdanDir, config, fileTrackingCallback, results) {
    ...
    for (const agentFile of agentFiles) {
      ...
      // Use the actual mdanDir if available (for when installing to temp dir)
      const actualMdanDir = config._mdanDir || mdanDir;
      const customizePath = path.join(actualMdanDir, '_config', 'agents', `custom-${agentName}.customize.yaml`);
