...
class Installer {
   ...
   * Checks mdan/config.yaml first, then other module configs
  ...
  async _readOutputFolder(mdanDir) {
    ...
    // Check mdan/config.yaml first (most common)
    ...
    try {
      ...
      for (const entry of entries) {
        if (!entry.isDirectory() || entry.name === 'mdan' || entry.name === 'mdan' || entry.name.startsWith('_')) continue;
