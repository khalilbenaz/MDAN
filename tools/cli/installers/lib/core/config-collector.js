...
class ConfigCollector {
   ...
   * @returns {Promise<string>} Path to mdan directory
   */
  async findMdanDir(projectDir) {
    // Check if project directory exists
    if (!(await fs.pathExists(projectDir))) {
      // Project doesn't exist yet, return default
      return path.join(projectDir, 'mdan');
    }
  ...
  /**
   * Detect the existing MDAN folder name in a project
   * @param {string} projectDir - Project directory
   * @returns {Promise<string|null>} Folder name (just the name, not full path) or null if not found
   */
  async detectExistingMdanFolder(projectDir) {
    // Check if project directory exists
    if (!(await fs.pathExists(projectDir))) {
      return null;
    }
  ...
  async loadExistingConfig(projectDir) {
    ...
    // Find the actual mdan directory (handles custom folder names)
    ...
    // Dynamically discover all installed modules by scanning mdan directory
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        const moduleConfigPath = path.join(mdanDir, entry.name, 'config.yaml');
