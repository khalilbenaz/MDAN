...
class Manifest {
   ...
   */
  async create(mdanDir, data, installedFiles = []) {
    ...
    // Get the MDAN version from package.json
    const mdanVersion = data.version || require(path.join(process.cwd(), 'package.json')).version;

    // Convert module list to new detailed format
    const moduleDetails = [];
    if (data.modules && Array.isArray(data.modules)) {
      for (const moduleName of data.modules) {
        // Core and BMM modules use the MDAN version
        const moduleVersion = moduleName === 'core' || moduleName === 'bmm' || moduleName === 'mdan' ? mdanVersion : null;
        ...
        moduleDetails.push({
          ...
          installDate: now,
          lastUpdated: now,
          source: moduleName === 'core' || moduleName === 'bmm' || moduleName === 'mdan' ? 'built-in' : 'unknown',
        });
      }
    }
   ...
   */
  async read(mdanDir) {
    const yamlPath = path.join(mdanDir, '_config', 'manifest.yaml');
    const yaml = require('yaml');
   ...
   */
  async update(mdanDir, updates, installedFiles = null) {
    const yaml = require('yaml');
    const manifest = (await this._readRaw(mdanDir)) || {
      installation: {},
      modules: [],
      ides: [],
    };
   ...
   */
  async addModule(mdanDir, moduleName, options = {}) {
    const manifest = await this._readRaw(mdanDir);
    if (!manifest) {
    ...
    await this._writeRaw(mdanDir, manifest);
  }
   ...
   */
  async removeModule(mdanDir, moduleName) {
    const manifest = await this._readRaw(mdanDir);
   ...
   */
  async updateModuleVersion(mdanDir, moduleName, versionInfo) {
    const manifest = await this._readRaw(mdanDir);
    if (!manifest || !manifest.modules) {
      return;
    }
   ...
   */
  async getModuleVersion(mdanDir, moduleName) {
    const manifest = await this._readRaw(mdanDir);
    if (!manifest || !manifest.modules) {
      return null;
    }
   ...
   */
  async getAllModuleVersions(mdanDir) {
    const manifest = await this._readRaw(mdanDir);
    if (!manifest || !manifest.modules) {
      return [];
    }
   ...
   */
  async _writeRaw(mdanDir, manifestData) {
    const yaml = require('yaml');
    const manifestPath = path.join(mdanDir, '_config', 'manifest.yaml');
   ...
   */
  async addIde(mdanDir, ideName) {
    const manifest = await this.read(mdanDir);
    if (!manifest) {
      throw new Error('No manifest found');
    }
    ...
    if (!manifest.ides.includes(ideName)) {
      ...
      await this.update(mdanDir, { ides: manifest.ides });
    }
  }
   ...
   * @param {string} mdanDir - Path to mdan directory for relative paths
   * @returns {Array} Array of file metadata objects
   */
  async parseInstalledFiles(installedFiles, mdanDir) {
    const fileMetadata = [];

    for (const filePath of installedFiles) {
      const fileExt = path.extname(filePath).toLowerCase();
      // Make path relative to parent of mdan directory, starting with 'mdan/'
      const relativePath = 'mdan' + filePath.replace(mdanDir, '').replaceAll('\\', '/');
   ...
   * @param {string} relativePath - Relative path starting with 'mdan/'
   ...
   */
  async addCustomModule(mdanDir, customModule) {
    const manifest = await this.read(mdanDir);
    if (!manifest) {
      throw new Error('No manifest found');
    }
    ...
    await this.update(mdanDir, { customModules: manifest.customModules });
  }
   ...
   */
  async removeCustomModule(mdanDir, moduleId) {
    const manifest = await this.read(mdanDir);
   ...
   */
  async getModuleVersionInfo(moduleName, mdanDir, moduleSourcePath = null) {
    const os = require('node:os');
    const yaml = require('yaml');

    // Built-in modules use MDAN version (only core and bmm are in MDAN repo)
    if (['core', 'bmm', 'mdan'].includes(moduleName)) {
    ...
    if (moduleInfo) {
      ...
      if (!version) {
        const cacheDir = path.join(os.homedir(), '.mdan', 'cache', 'external-modules', moduleName);
    ...
    const cacheDir = path.join(mdanDir, '_config', 'custom', moduleName);
   ...
   */
  async checkForUpdates(mdanDir) {
    const modules = await this.getAllModuleVersions(mdanDir);
    const updates = [];
