...
/**
 * Dependency Resolver for MDAN modules
...
class DependencyResolver {
   ...
   */
  async resolve(mdanDir, selectedModules = [], options = {}) {
    if (options.verbose) {
    ...
    const primaryFiles = await this.collectPrimaryFiles(mdanDir, modulesToProcess, options);
    ...
    const resolvedDeps = await this.resolveDependencyPaths(mdanDir, allDependencies);

    // Fourth pass: check for transitive dependencies
    const transitiveDeps = await this.resolveTransitiveDependencies(mdanDir, resolvedDeps);

    // Combine all files
    const allFiles = new Set([...primaryFiles.map((f) => f.path), ...resolvedDeps, ...transitiveDeps]);

    // Organize by module
    const organizedFiles = this.organizeByModule(mdanDir, allFiles);
   ...
   */
  async collectPrimaryFiles(mdanDir, modules, options = {}) {
    const files = [];
    ...
    for (const module of modules) {
      ...
      // Handle both source (src/) and installed (mdan/) directory structures
      let moduleDir;

      // Check if this is a source directory (has 'src' subdirectory)
      const srcDir = path.join(mdanDir, 'src');
  ...
  async parseDependencies(files) {
    ...
    for (const file of files) {
      ...
      for (const ref of fileRefs) {
        ...
        // Paths starting with mdan/ are absolute references to the mdan installation
        const depType = ref.startsWith('mdan/') ? 'mdan-path' : 'file';
        allDeps.add({
          from: file.path,
          dependency: ref,
          type: depType,
  ...
  parseCommandReferences(content) {
    ...
    const commandPattern = /@(task-|agent-|mdan-)([a-z0-9-]+)/g;
    let match;
    ...
    // Match file paths like mdan/core/agents/analyst
    const pathPattern = /mdan\/(core|mdan|cis)\/(agents|tasks)\/([a-z0-9-]+)/g;
   ...
   */
  async resolveDependencyPaths(mdanDir, dependencies) {
    const resolved = new Set();

    for (const dep of dependencies) {
      const resolvedPaths = await this.resolveSingleDependency(mdanDir, dep);
   ...
   */
  async resolveSingleDependency(mdanDir, dep) {
    const paths = [];
    ...
    switch (dep.type) {
      ...
      case 'file': {
        ...
        if (depPath.includes('{project-root}')) {
          // Remove {project-root} and resolve as mdan path
          depPath = depPath.replace('{project-root}', '');

          if (depPath.startsWith('mdan/')) {
            const mdanPath = depPath.replace(/^mdan\//, '');

            // Handle glob patterns
            if (depPath.includes('*')) {
              // Extract the base path and pattern
              const pathParts = mdanPath.split('/');
              ...
              let basePath;
              if (module === 'core') {
                basePath = path.join(mdanDir, 'core', middlePath);
              } else {
                basePath = path.join(mdanDir, 'modules', module, middlePath);
              }
              ...
              if (mdanPath.startsWith('core/')) {
                const corePath = path.join(mdanDir, mdanPath);
                if (await fs.pathExists(corePath)) {
                  paths.push(corePath);
                }
              } else {
                const parts = mdanPath.split('/');
                const module = parts[0];
                const rest = parts.slice(1).join('/');
                const modulePath = path.join(mdanDir, 'modules', module, rest);
      ...
      case 'command': {
        ...
        const commandPath = await this.resolveCommandToPath(mdanDir, dep.dependency);
        if (commandPath) {
      ...
      case 'mdan-path': {
        ...
        if (mdanPath.startsWith('core/')) {
          ...
          // It's a module path like mdan/tasks/foo.md or cis/agents/bar.md
          const parts = mdanPath.split('/');
          const module = parts[0];
          const rest = parts.slice(1).join('/');
          let modulePath;
          if (module === 'mdan') {
            ...
            modulePath = path.join(mdanDir, 'modules', module, rest);
          }
      ...
      case 'template': {
        ...
        if (templateDep.includes('{project-root}')) {
          // Remove {project-root} and treat as mdan-path
          templateDep = templateDep.replace('{project-root}', '');

          // Now resolve as a mdan path
          if (templateDep.startsWith('mdan/')) {
            const mdanPath = templateDep.replace(/^mdan\//, '');
   ...
   */
  async resolveCommandToPath(mdanDir, command) {
    // Parse command format: @task-name or @agent-name or mdan/module/type/name

    if (command.startsWith('@task-')) {
      const taskName = command.slice(6);
      // Search all modules for this task
      for (const module of ['core', 'mdan', 'cis']) {
        const taskPath =
          module === 'core'
            ? path.join(mdanDir, 'core', 'tasks', `${taskName}.md`)
            : path.join(mdanDir, 'modules', module, 'tasks', `${taskName}.md`);
      ...
      for (const module of ['core', 'mdan', 'cis']) {
        const agentPath =
          module === 'core'
            ? path.join(mdanDir, 'core', 'agents', `${agentName}.md`)
            : path.join(mdanDir, 'modules', module, 'agents', `${agentName}.md`);
        if (await fs.pathExists(agentPath)) {
          return agentPath;
        }
      }
    } else if (command.startsWith('mdan/')) {
      ...
      if (parts.length >= 4) {
        ...
        const filePath =
          module === 'core' ? path.join(mdanDir, 'core', type, fileName) : path.join(mdanDir, 'modules', module, type, fileName);
   ...
   */
  async resolveTransitiveDependencies(mdanDir, directDeps) {
        ...
        const subDeps = await this.parseDependencies([
          {
            path: depPath,
            type: 'dependency',
            module: this.getModuleFromPath(mdanDir, depPath),
            name: path.basename(depPath),
          },
        ]);

        const resolvedSubDeps = await this.resolveDependencyPaths(mdanDir, subDeps);
   ...
   */
  getModuleFromPath(mdanDir, filePath) {
    const relative = path.relative(mdanDir, filePath);
    const parts = relative.split(path.sep);

    // Handle source directory structure (src/core, src/mdan, or src/modules/xxx)
    if (parts[0] === 'src') {
      if (parts[1] === 'core') {
        return 'core';
      } else if (parts[1] === 'mdan') {
        return 'mdan';
   ...
   */
  organizeByModule(mdanDir, files) {
    const organized = {};

    for (const file of files) {
      const module = this.getModuleFromPath(mdanDir, file);
      if (!organized[module]) {
        organized[module] = {
          agents: [],
          tasks: [],
      ...
      if (file.includes('/src/core/') || file.includes('/src/mdan/')) {
        ...
        moduleBase = module === 'core' ? path.join(mdanDir, 'core') : path.join(mdanDir, 'modules', module);
      }
