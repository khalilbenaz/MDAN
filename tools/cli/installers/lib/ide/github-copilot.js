...
`;
  ...
  createWorkflowPromptContent(entry, workflowFile, toolsStr) {
    ...
    // mdan/config.yaml is safe to hardcode here: these prompts are only generated when
    // mdan-help.csv exists (mdan module data), so mdan is guaranteed to be installed.
    const configLine = `1. Load {project-root}/${this.mdanFolderName}/mdan/config.yaml and store ALL fields as session variables`;
...
`;
  ...
  createTechWriterPromptContent(entry) {
    ...
    const techWriterCommands = {
      'Write Document': { code: 'WD', file: 'mdan-mdan-write-document', description: 'Write document' },
      'Update Standards': { code: 'US', file: 'mdan-mdan-update-standards', description: 'Update standards' },
      'Mermaid Generate': { code: 'MG', file: 'mdan-mdan-mermaid-generate', description: 'Mermaid generate' },
      'Validate Document': { code: 'VD', file: 'mdan-mdan-validate-document', description: 'Validate document' },
      'Explain Concept': { code: 'EC', file: 'mdan-mdan-explain-concept', description: 'Explain concept' },
    };
...
1. Load {project-root}/${this.mdanFolderName}/mdan/config.yaml and store ALL fields as session variables
2. Load the full agent file from {project-root}/${this.mdanFolderName}/mdan/agents/tech-writer/tech-writer.md and activate the Paige (Technical Writer) persona
...
`;
  ...
  createAgentActivatorPromptContent(artifact, manifestEntry, toolsStr) {
    ...
    // mdan/config.yaml is safe to hardcode: agent activators are only generated from
    // mdan agent artifacts, so mdan is guaranteed to be installed.
    return `---
description: '${safeDescription}'
agent: 'agent'
tools: ${toolsStr}
...
- **Agent definitions**: \`${mdan}/mdan/agents/\` (MDAN module) and \`${mdan}/core/agents/\` (core)
- **Workflow definitions**: \`${mdan}/mdan/workflows/\` (organized by phase)
...
- **Module configuration**: \`${mdan}/mdan/config.yaml\`
...
- Always load \`${mdan}/mdan/config.yaml\` before any agent activation or workflow execution
...
Type \`/mdan-\` in Copilot Chat to see all available MDAN workflows and agent activators. Agents are also available in the agents dropdown.`;
  ...
  async loadModuleConfig(mdanDir) {
    const bmmConfigPath = path.join(mdanDir, 'mdan', 'config.yaml');
   ...
   * @param {string} fileName - Target filename (e.g. 'mdan-agent-mdan-pm.agent.md')
