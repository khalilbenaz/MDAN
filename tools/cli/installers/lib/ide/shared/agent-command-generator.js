...
class AgentCommandGenerator {
  ...
  async collectAgentArtifacts(mdanDir, selectedModules = []) {
    ...
    for (const agent of agents) {
      ...
      // Calculate the relative agent path (e.g., mdan/agents/pm.md)
  ...
  async writeColonArtifacts(baseCommandsDir, artifacts) {
    ...
    for (const artifact of artifacts) {
      if (artifact.type === 'agent-launcher') {
        // Convert relativePath to underscore format: mdan/agents/pm.md → mdan_bmm_pm.md
   ...
   * Creates flat files like: mdan-agent-mdan-pm.md
   *
  ...
  async writeDashArtifacts(baseCommandsDir, artifacts) {
    ...
    for (const artifact of artifacts) {
      if (artifact.type === 'agent-launcher') {
        // Convert relativePath to dash format: mdan/agents/pm.md → mdan-agent-mdan-pm.md
