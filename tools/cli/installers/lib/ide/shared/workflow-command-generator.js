...
class WorkflowCommandGenerator {
  ...
  async collectWorkflowArtifacts(mdanDir) {
    ...
    for (const workflow of allWorkflows) {
      const commandContent = await this.generateCommandContent(workflow, mdanDir);
      // Calculate the relative workflow path (e.g., mdan/workflows/4-implementation/sprint-planning/workflow.yaml)
      ...
      if (workflowRelPath.includes('_.mdan/')) {
        ...
        // Normalize source paths (e.g. .../src/mdan/...) to relative module path (e.g. mdan/...)
  ...
  async generateCommandContent(workflow, mdanDir) {
    ...
    if (workflowPath.includes('/src/mdan/')) {
      // mdan is directly under src/
      const match = workflowPath.match(/\/src\/mdan\/(.+)/);
      if (match) {
        workflowPath = `${this.mdanFolderName}/mdan/${match[1]}`;
      }
...
`;
  ...
  transformWorkflowPath(workflowPath) {
    ...
    if (workflowPath.includes('/src/mdan/')) {
      const match = workflowPath.match(/\/src\/mdan\/(.+)/);
      if (match) {
        transformed = `{project-root}/${this.mdanFolderName}/mdan/${match[1]}`;
      }
    } else if (workflowPath.includes('/src/core/')) {
      const match = workflowPath.match(/\/src\/core\/(.+)/);
      if (match) {
  ...
  async writeColonArtifacts(baseCommandsDir, artifacts) {
    ...
    for (const artifact of artifacts) {
      if (artifact.type === 'workflow-command') {
        // Convert relativePath to underscore format: mdan/workflows/correct-course.md → mdan_bmm_correct-course.md
   ...
   * Creates flat files like: mdan-mdan-correct-course.md
   *
  ...
  async writeDashArtifacts(baseCommandsDir, artifacts) {
    ...
    for (const artifact of artifacts) {
      if (artifact.type === 'workflow-command') {
        // Convert relativePath to dash format: mdan/workflows/correct-course.md → mdan-mdan-correct-course.md
