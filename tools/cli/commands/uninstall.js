...
module.exports = {
  command: 'uninstall',
  description: 'Remove MDAN installation from the current project',
  options: [
    ['-y, --yes', 'Remove all MDAN components without prompting (preserves user artifacts)'],
    ...
    try {
      ...
      if (options.directory) {
        ...
        const dirChoice = await prompts.select({
          message: 'Where do you want to uninstall MDAN from?',
          choices: [
      ...
      if (!(await fs.pathExists(mdanDir))) {
        await prompts.log.warn('No MDAN installation found.');
        process.exit(0);
      }
      ...
      await prompts.intro('MDAN Uninstall');
      ...
      if (!options.yes) {
        ...
        const selected = await prompts.multiselect({
          ...
          options: [
            {
              value: 'modules',
              label: `MDAN Modules & data (${installer.mdanFolderName}/)`,
