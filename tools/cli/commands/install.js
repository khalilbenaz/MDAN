...
module.exports = {
  command: 'install',
  description: 'Install MDAN Core agents and tools',
  options: [
    ['-d, --debug', 'Enable debug output for manifest generation'],
    ['--directory <path>', 'Installation directory (default: current directory)'],
    ['--modules <modules>', 'Comma-separated list of module IDs to install (e.g., "mdan,bmb")'],
    [
      '--tools <tools>',
  ...
  action: async (options) => {
    try {
      ...
      if (options.debug) {
        process.env.MDAN_DEBUG_MANIFEST = 'true';
