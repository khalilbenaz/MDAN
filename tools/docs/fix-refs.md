...
description: Corrects workflow, agent, and command references in MDAN documentation
---
...
Fix incorrect references to workflows, agents, and commands in MDAN documentation files.
...
**How-To guides require judgment**: Don't assume experienced. Ask: "Does this task require prior MDAN knowledge?" Early-journey tasks (first PRD, first sprint) are newbie docs. Customization and advanced features are experienced.

**If unclear**: Ask the user "Who is the target audience for this document — new users learning MDAN, or experienced users who know the system?"
...
| `mdan_bmm_*` | `mdan_bmm_workflow-init` | Internal slash command name, platform-specific |
...
- First mention: Run the `help` workflow (`/mdan-help` on most platforms)
...
|----------|----------|-------|
| `workflow-init` | `mdan-help` | DEPRECATED - help system replaces initialization |
| `workflow-status` | `mdan-help` | DEPRECATED - help system replaces status checking |

### The Help System

The `mdan-help` workflow is the modern replacement for both `workflow-init` and `workflow-status`:
...
Users can run `mdan-help` to get guidance on what to do next. It detects:
...
3. **Simple names**: Just the workflow name, no `mdan_bmm_` prefix, no `/` prefix
