...
### Removed
- Comparison page with a third-party framework and every reference to it in the documentation.
...
- `mdan validate` also checks every `path` column of the content CSVs; tests guard unique persona names and real party members.
...
- Documentation: English README (`README.en.md`, generated sections included), docs reorganised by purpose (tutorial, how-to guides, CLI/MCP/modules/config reference, concepts).
- Dependencies: MCP SDK 1.30, zod 4, ESLint 10 (rethrown errors keep their `cause`), actions/checkout and setup-node v7.

### Fixed
- Legacy leftovers: party team and help tables referenced agents and files that do not exist; duplicate persona names across modules renamed; hardcoded French-Darija mandates removed from ecosystem agents.
