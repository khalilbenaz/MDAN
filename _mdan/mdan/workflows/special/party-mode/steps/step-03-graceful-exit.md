...
"What an incredible collaborative session! Thank you {{user_name}} for engaging with our MDAN agent team in this dynamic discussion. Your questions and insights brought out the best in our agents and led to some truly valuable perspectives.
...
[Bash: .claude/hooks/mdan-speak.sh \"[Agent Name]\" \"[Their farewell message]\"]"
...
Thank you for bringing our MDAN agents together in this unique collaborative experience. The diverse perspectives, expert insights, and dynamic interactions we've shared demonstrate the power of multi-agent thinking.
...
### 5. Persist Agent Memory (Sidecars)

Follow the memory persistence protocol in `../protocols/memory-protocol.md`:

**For each participating agent:**

1. Load existing sidecar from `{project-root}/_mdan/state/sidecars/{agent-name}.sidecar.json` (or create new)
2. Increment `sessions_participated`
3. Add new memories from this session (max 5 per session):
   - Key observations the agent made
   - Preferences expressed
   - Decisions participated in
4. Update `relationships` based on agreements/disagreements
5. Add decision history entries for any Decision Records created
6. Save updated sidecar

"📝 **Agent memories persisted:**
[For each agent with updates:]
- [Icon] [Agent Name]: +X memories saved (Y total)"

### 6. Register Decisions in Context Graph

If any Decision Records were created during this session:

1. Load context graph from `{project-root}/_mdan/state/context-graph.json`
2. For each Decision Record:
   - Add as node (type: "decision", path: decision record file)
   - Link to related artifacts with appropriate edge relations
   - Mark `registered_in_graph: true` in the DR file
3. Save updated context graph

### 7. Update MDAN-STATE

Update `{project-root}/_mdan/state/MDAN-STATE.json`:
- Add session to `sessions[]` with date, mode, participants, decisions
- Update `agent_sidecars{}` with latest sidecar metadata
- Add any new decisions to `decisions[]`

### 8. Complete Workflow Exit
...
Thank you for using MDAN Party Mode for collaborative multi-agent discussions!"
...
Congratulations on facilitating a successful multi-agent collaborative discussion through MDAN Party Mode! 🎉
