...
[Bash: .claude/hooks/mdan-speak.sh \"[Agent Name]\" \"[Their response]\"]"
...
### 6. Turn Structure (Enhanced)

Follow the turn protocol defined in `../protocols/turn-protocol.md`:
- Each agent response uses structured turn markers
- Word limits enforced per turn type
- No agent speaks twice consecutively
- Topic drift detection: if 2+ turns are off-topic, facilitator redirects

### 7. Response Round Completion

After generating all agent responses for the round, let the user know he can speak naturally with the agents, then show this menu:

"[D] Switch to **Debate** mode | [C] Switch to **Consensus** mode | [E] Exit Party Mode"

### 8. Mode Switching

#### If 'D' (Switch to Debate):
- Update frontmatter: `orchestration_mode: 'debate'`
- Load: `./step-02a-debate-mode.md`

#### If 'C' (Switch to Consensus):
- Update frontmatter: `orchestration_mode: 'consensus'`
- Load: `./step-02b-consensus-mode.md`

### 9. Exit Condition Checking
