...
Use the **DEV agent** directly for bug fixes, refactorings, or small targeted changes that don't require the full MDAN Method or Quick Flow.
...
```text
/mdan-agent-mdan-quick-flow-solo-dev
```
...
Consider using [Quick Flow](../explanation/quick-flow.md) or the full MDAN Method when:
