...
Each workflow has a **slash command** you run in your IDE (e.g., `/mdan-mdan-create-prd`). Running a workflow command automatically loads the appropriate agent — you don't need to load agents separately. You can also load an agent directly for general conversation (e.g., `/mdan-agent-mdan-pm` for the PM agent).
:::
...
Create it manually at `_mdan-output/project-context.md` or generate it after architecture using `/mdan-mdan-generate-project-context`. [Learn more](../explanation/project-context.md).
:::
...
- **research** (`/mdan-mdan-research`) — Market and technical research
- **create-product-brief** (`/mdan-mdan-create-product-brief`) — Recommended foundation document

### Phase 2: Planning (Required)

**For MDAN Method and Enterprise tracks:**
1. Load the **PM agent** (`/mdan-agent-mdan-pm`) in a new chat
2. Run the `prd` workflow (`/mdan-mdan-create-prd`)
3. Output: `PRD.md`

**For Quick Flow track:**
- Use the `quick-spec` workflow (`/mdan-mdan-quick-spec`) instead of PRD, then skip to implementation

:::note[UX Design (Optional)]
If your project has a user interface, load the **UX-Designer agent** (`/mdan-agent-mdan-ux-designer`) and run the UX design workflow (`/mdan-mdan-create-ux-design`) after creating your PRD.
:::

### Phase 3: Solutioning (MDAN Method/Enterprise)

**Create Architecture**
1. Load the **Architect agent** (`/mdan-agent-mdan-architect`) in a new chat
2. Run `create-architecture` (`/mdan-mdan-create-architecture`)
...
1. Load the **PM agent** (`/mdan-agent-mdan-pm`) in a new chat
2. Run `create-epics-and-stories` (`/mdan-mdan-create-epics-and-stories`)
3. The workflow uses both PRD and Architecture to create technically-informed stories

**Implementation Readiness Check** *(Highly Recommended)*
1. Load the **Architect agent** (`/mdan-agent-mdan-architect`) in a new chat
2. Run `check-implementation-readiness` (`/mdan-mdan-check-implementation-readiness`)
...
Load the **SM agent** (`/mdan-agent-mdan-sm`) and run `sprint-planning` (`/mdan-mdan-sprint-planning`). This creates `sprint-status.yaml` to track all epics and stories.
...
| ---- | ----- | -------------- | -------------------------- | ---------------------------------- |
| 1    | SM    | `create-story` | `/mdan-mdan-create-story`  | Create story file from epic        |
| 2    | DEV   | `dev-story`    | `/mdan-mdan-dev-story`     | Implement the story                |
| 3    | DEV   | `code-review`  | `/mdan-mdan-code-review`   | Quality validation *(recommended)* |

After completing all stories in an epic, load the **SM agent** (`/mdan-agent-mdan-sm`) and run `retrospective` (`/mdan-mdan-retrospective`).
...
| `prd`                                 | `/mdan-mdan-create-prd`                     | PM        | Create Product Requirements Document            |
| `create-architecture`                 | `/mdan-mdan-create-architecture`            | Architect | Create architecture document                     |
| `generate-project-context`            | `/mdan-mdan-generate-project-context`       | Analyst   | Create project context file                     |
| `create-epics-and-stories`       | `/mdan-mdan-create-epics-and-stories`       | PM        | Break down PRD into epics            |
| `check-implementation-readiness` | `/mdan-mdan-check-implementation-readiness` | Architect | Validate planning cohesion           |
| `sprint-planning`                | `/mdan-mdan-sprint-planning`                | SM        | Initialize sprint tracking           |
| `create-story`                   | `/mdan-mdan-create-story`                   | SM        | Create a story file                  |
| `dev-story`                      | `/mdan-mdan-dev-story`                      | DEV       | Implement a story                    |
| `code-review`                    | `/mdan-mdan-code-review`                    | DEV       | Review implemented code              |
...
Yes. The SM agent has a `correct-course` workflow (`/mdan-mdan-correct-course`) for handling scope changes.

**What if I want to brainstorm first?**
Load the Analyst agent (`/mdan-agent-mdan-analyst`) and run `brainstorming` (`/mdan-brainstorming`) before starting your PRD.
