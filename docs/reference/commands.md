...
```text
.claude/commands/
├── mdan-agent-mdan-dev.md
├── mdan-agent-mdan-pm.md
├── mdan-mdan-create-prd.md
├── mdan-editorial-review-prose.md
├── mdan-help.md
└── ...
```

The filename determines the slash command name in your IDE. For example, the file `mdan-agent-mdan-dev.md` registers the command `/mdan-agent-mdan-dev`.
...
| --- | --- | --- |
| `/mdan-agent-mdan-dev` | Amelia (Developer) | Implements stories with strict adherence to specs |
| `/mdan-agent-mdan-pm` | John (Product Manager) | Creates and validates PRDs |
| `/mdan-agent-mdan-architect` | Winston (Architect) | Designs system architecture |
| `/mdan-agent-mdan-sm` | Bob (Scrum Master) | Manages sprints and stories |
...
| --- | --- |
| `/mdan-mdan-create-prd` | Create a Product Requirements Document |
| `/mdan-mdan-create-architecture` | Design system architecture |
| `/mdan-mdan-dev-story` | Implement a story |
| `/mdan-mdan-code-review` | Run a code review |
| `/mdan-mdan-quick-spec` | Define an ad-hoc change (Quick Flow) |
...
| --- | --- | --- |
| `mdan-agent-<module>-<name>` | Agent launcher | `mdan-agent-mdan-dev` |
| `mdan-<module>-<workflow>` | Workflow command | `mdan-mdan-create-prd` |
| `mdan-<name>` | Core task or tool | `mdan-help` |

Module codes: `mdan` (Agile suite), `bmb` (Builder), `tea` (Test Architect), `cis` (Creative Intelligence), `gds` (Game Dev Studio). See [Modules](./modules.md) for descriptions.
