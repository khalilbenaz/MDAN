...
MDAN provides two testing paths: a built-in QA agent for fast test generation and an installable Test Architect module for enterprise-grade test strategy.
...
| **Setup** | Nothing to install -- included in MDAN | Install separately via `npx mdan-method install` |
...
Quinn is the built-in QA agent in the MDAN (Agile suite) module. It generates working tests quickly using your project's existing test framework -- no configuration or additional installation required.

**Trigger:** `QA` or `mdan-mdan-qa-automate`
...
- **Documentation:** [TEA Module Docs](https://mdan-code-org.github.io/mdan-method-test-architecture-enterprise/)
...
Quinn's Automate workflow appears in Phase 4 (Implementation) of the MDAN Method workflow map. A typical sequence:
